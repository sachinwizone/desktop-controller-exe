...
const freshAdmin = {
    ...
    loadWebLogs() {
        ...
        fetch(`/api.php?action=get_web_logs&company_name=${encodeURIComponent(companyName)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data && data.data.length > 0) {
                    const logsHtml = data.data.map(log => {
                        const durationMin = Math.floor((log.duration_seconds || 0) / 60);
                        const timestamp = new Date(log.visit_time).toLocaleString('en-IN');
                        
                        return `
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td style="padding: 16px; color: #1e293b; font-weight: 500;">${log.display_user_name || log.username || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${log.website_url || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${log.page_title || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${durationMin} min</td>
                                <td style="padding: 16px; color: #64748b;">${timestamp}</td>
                            </tr>
                        `;
                    }).join('');
                    ...
                    tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #64748b;">No web activity found</td></tr>';
                }
            })
            .catch(error => {
                console.error('Error loading web logs:', error);
                tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #dc2626;">Error loading web logs</td></tr>';
            });
    },
    ...
    loadAppLogs() {
        ...
        fetch(`/api.php?action=get_application_logs&company_name=${encodeURIComponent(companyName)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data && data.data.length > 0) {
                    const appsHtml = data.data.map(app => {
                        const durationMin = Math.floor((app.duration_seconds || 0) / 60);
                        const startTime = new Date(app.start_time).toLocaleString('en-IN');
                        const status = app.is_active ? 'Active' : 'Closed';
                        
                        return `
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td style="padding: 16px; color: #1e293b; font-weight: 500;">${app.display_user_name || app.username || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${app.app_name || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${app.window_title || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${durationMin} min</td>
                                <td style="padding: 16px;">
                                    <span style="padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 500; background: ${status === 'Active' ? '#fef3c7' : '#f3f4f6'}; color: ${status === 'Active' ? '#92400e' : '#6b7280'};">${status}</span>
                                </td>
                            </tr>
                        `;
                    }).join('');
                    
                    tbody.innerHTML = appsHtml;
                } else {
                    tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #64748b;">No application usage found</td></tr>';
                }
            })
            .catch(error => {
                console.error('Error loading application logs:', error);
                tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #dc2626;">Error loading application logs</td></tr>';
            });
    },
    ...
    loadInactivityLogs() {
        ...
        fetch(`/api.php?action=get_inactivity_logs&company_name=${encodeURIComponent(companyName)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data && data.data.length > 0) {
                    const inactivityHtml = data.data.map(item => {
                        const durationMin = Math.floor((item.duration_seconds || 0) / 60);
                        const startTime = new Date(item.start_time).toLocaleString('en-IN');
                        
                        return `
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                <td style="padding: 16px; color: #1e293b; font-weight: 500;">${item.display_user_name || item.username || 'N/A'}</td>
                                <td style="padding: 16px; color: #64748b;">${startTime}</td>
                                <td style="padding: 16px; color: #64748b;">${durationMin} min</td>
                                <td style="padding: 16px; color: #64748b;">${item.system_name || 'N/A'}</td>
                                <td style="padding: 16px;">
                                    <span style="padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: 500; background: #dcfce7; color: #166534;">${item.status || 'Active'}</span>
                                </td>
                            </tr>
                        `;
                    }).join('');
                    
                    tbody.innerHTML = inactivityHtml;
                } else {
                    tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #64748b;">No inactivity records found</td></tr>';
                }
            })
            .catch(error => {
                console.error('Error loading inactivity logs:', error);
                tbody.innerHTML = '<tr><td colspan="5" style="padding: 24px; text-align: center; color: #dc2626;">Error loading inactivity logs</td></tr>';
            });
    },
    ...
    loadScreenshots() {
        ...
        fetch(`/api.php?action=get_screenshots&company_name=${encodeURIComponent(companyName)}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data && data.data.length > 0) {
                    const screenshotsHtml = data.data.map(shot => {
                        const timestamp = new Date(shot.capture_time).toLocaleString('en-IN');
                        
                        return `
                            <div style="display: inline-block; margin: 12px; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background: white;">
                                <div style="width: 200px; height: 150px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; color: #64748b;">
                                    <div style="text-align: center;">
                                        <div style="font-size: 24px; margin-bottom: 8px;">📸</div>
                                        <div style="font-size: 12px;">Screenshot</div>
                                    </div>
                                </div>
                                <div style="padding: 12px;">
                                    <div style="font-weight: 500; color: #1e293b; margin-bottom: 4px;">${shot.display_user_name || shot.username || 'N/A'}</div>
                                    <div style="font-size: 12px; color: #64748b;">${timestamp}</div>
                                    <button onclick="alert('Screenshot viewer - ${shot.system_name || 'System'}')" style="margin-top: 8px; padding: 6px 12px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 12px; width: 100%;">View</button>
                                </div>
                            </div>
                        `;
                    }).join('');
                    ...
                    grid.innerHTML = '<div style="text-align: center; color: #64748b; padding: 40px;">No screenshots found for selected criteria</div>';
                }
            })
            .catch(error => {
                console.error('Error loading screenshots:', error);
                grid.innerHTML = '<div style="text-align: center; color: #dc2626; padding: 40px;">Error loading screenshots</div>';
            });
    },
