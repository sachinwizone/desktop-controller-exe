...
const apiHandlers = {
    ...
    async get_web_logs(query) {
        try {
            ...
            let sql = `SELECT id, log_timestamp, system_name, username, browser_name, website_url, page_title, category, visit_time, duration_seconds, ip_address, display_user_name FROM web_logs WHERE company_name = $1`;
            let params = [companyName];
            let paramIndex = 2;
            
            if (query.date) {
                sql += ` AND DATE(visit_time) = $${paramIndex}`;
                params.push(query.date);
                paramIndex++;
            }
            
            if (query.search) {
                sql += ` AND (website_url ILIKE $${paramIndex} OR page_title ILIKE $${paramIndex} OR browser_name ILIKE $${paramIndex})`;
            ...
            sql += ' ORDER BY visit_time DESC LIMIT 500';
    ...
    async get_application_logs(query) {
        try {
            ...
            let sql = `SELECT id, log_timestamp, system_name, username, app_name, window_title, start_time, end_time, duration_seconds, is_active, ip_address, display_user_name FROM application_logs WHERE company_name = $1`;
            let params = [companyName];
            let paramIndex = 2;
            ...
            if (query.search) {
                sql += ` AND (app_name ILIKE $${paramIndex} OR window_title ILIKE $${paramIndex})`;
                params.push(`%${query.search}%`);
                paramIndex++;
            }
    ...
    async get_inactivity_logs(query) {
        try {
            ...
            let sql = `SELECT id, log_timestamp, system_name, username, start_time, end_time, duration_seconds, status, ip_address, display_user_name FROM inactivity_logs WHERE company_name = $1`;
            let params = [companyName];
            let paramIndex = 2;
            ...
            if (query.search) {
                sql += ` AND (system_name ILIKE $${paramIndex} OR username ILIKE $${paramIndex})`;
                params.push(`%${query.search}%`);
                paramIndex++;
            }
    ...
    async get_screenshots(query) {
        try {
            ...
            let sql = `SELECT id, log_timestamp, system_name, username, log_timestamp as capture_time, screen_width, screen_height, ip_address, display_user_name FROM screenshot_logs WHERE company_name = $1`;
            let params = [companyName];
            let paramIndex = 2;
            
            if (query.date) {
                sql += ` AND DATE(log_timestamp AT TIME ZONE 'Asia/Kolkata') = $${paramIndex}`;
                params.push(query.date);
                paramIndex++;
            }
    ...
    async get_employees(query) {
        try {
            ...
            const rows = await queryDB(
                `SELECT id, employee_id, full_name, email, phone, department, designation, is_active, created_at 
    ...
    async add_employee(body) {
        try {
            const { company_name, employee_id, full_name, email, phone, department, designation } = body;
            ...
            const result = await queryDB(
                `INSERT INTO company_employees (company_name, employee_id, full_name, email, phone, department, designation, is_active, created_at, updated_at)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, true, NOW(), NOW())
                 RETURNING id`,
                [company_name, employee_id || '', full_name, email || '', phone || '', department || '', designation || '']
            );
    ...
    async update_employee(body) {
        try {
            const { id, company_name, employee_id, full_name, email, phone, department, designation, is_active } = body;
            ...
            await queryDB(
                `UPDATE company_employees 
                 SET employee_id = $1, full_name = $2, email = $3, phone = $4, department = $5, designation = $6, is_active = $7, updated_at = NOW()
                 WHERE id = $8 AND company_name = $9`,
                [employee_id || '', full_name, email || '', phone || '', department || '', designation || '', is_active !== false, id, company_name]
            );
